...
Sub oooGeneralRebrand()
    pStrDocType = "general"
    Call oooRebranding(GENERAL_HEADER, GENERAL_FOOTER)
End Sub

Sub oooIntakeRebrand()
    pStrDocType = "intake"
    Call oooRebranding(INTAKE_HEADER, INTAKE_FOOTER)
End Sub

Sub oooRebranding(headFile As String, footFile As String)
'
' oooRebranding Macro for rebranding
...
'
    ...
    ''' or REprocessing new docs. Once we've made it through all the old stuff, this can go away and can drop the old-line deleter
    Dim reProcess As Integer
    reProcess = MsgBox("is this a RE-process of NEW forms?", vbYesNo + vbQuestion, "reProcess?")
    If reProcess = vbNo Then
...
' That's a problem if for some reason we need to re-run
' the General macro, or run the Intake macro afterwards. So...
'
